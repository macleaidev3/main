...
ORIGINAL_FOLDER = Path(r"D:\Anurag BPCL WORK\Glaucosense\Evaluation of Fundus Images\Original_images")
# Folder containing ESRGAN processed images
ESRGAN_FOLDER = Path(r"D:\Anurag BPCL WORK\Glaucosense\Evaluation of Fundus Images\ESRGAN_Result")
# CSV output location
OUTPUT_CSV = Path(r"D:\Anurag BPCL WORK\Glaucosense\Evaluation of Fundus Images\ESRGAN_Result\ESRGAN_Quality_Evaluation.csv")
...
matched_files = sorted(set(original_images.keys()) & set(esrgan_images.keys()))
if not matched_files:
    raise RuntimeError("No matching filenames were found between the Original and ESRGAN folders.")
...
def calculate_quality_score(image):
    ...
    IMPORTANT:
        This is a relative image-quality index,
        not a universal absolute image-quality measurement.
    """
...
for index, filename in enumerate(matched_files, start=1):
    ...
    try:
        ...
        esrgan_quality_score = (quality_ratio * 100)
...
improved = (valid_results["Verdict"] == "IMPROVED").sum()
degraded = (valid_results["Verdict"] == "DEGRADED").sum()
unchanged = (valid_results["Verdict"] == "NO CHANGE").sum()
print("=" * 70)
...
RED_FLAG_FOLDER = ESRGAN_FOLDER / "Red Flag" #(Path(__file__).resolve().parent / "Red Flag")
...
red_flag_results = df[df["Quality Change (%)"].notna() & (df["Quality Change (%)"] <= 60)]
red_flag_count = 0
red_flag_errors = 0
for _, row in red_flag_results.iterrows():
