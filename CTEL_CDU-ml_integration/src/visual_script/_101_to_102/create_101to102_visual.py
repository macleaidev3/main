...
# from src.visual_script.icv112.visual_2d import ICV112Visual2d
# from src.visual_script.excel_visual.pipeline_3d_widget import Corrosion3DWidget
# from src.visual_script.excel_visual.plot_2d_widget import Corrosion2DWidget

from src.visual_script.visual_3d_pipeline.pipeline_3d_widget import DatabasePipeline3DWidget
from ui.widgets.ribbon_tab import RibbonTab

class Create101ton102Visual(RibbonTab):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.visual_blueprints = {
            # "view_2d": Corrosion2DWidget,
            # "view_3d": Corrosion3DWidget
            "view_3d": DatabasePipeline3DWidget
        }
    ...
    def switch_view(self, button_id: str):
        ...
        if button_id not in self.loaded_widgets:
            ...
            new_widget = widget_class(equipment_name="101_to_102")
            # new_widget = widget_class("visual/_101_to_102/101-102_all.xlsx")
        ...
        # Loading a new pyvista viewer can make the enclosing MDI subwindow
        # unmaximize as its layout recalculates around the new widget.
        # Re-assert the maximized state once the new view has settled.
        sub_window = self.window()
        if isinstance(sub_window, QtWidgets.QMdiSubWindow) and sub_window.isMaximized():
            QtCore.QTimer.singleShot(0, sub_window.showMaximized)