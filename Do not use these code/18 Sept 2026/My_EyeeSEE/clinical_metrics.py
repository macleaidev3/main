...
def run_sanity_checks(disc_mask: np.ndarray, cup_mask: np.ndarray) -> None:
    ...
    # 3. Cup ⊂ Disc  (every cup pixel must be a disc pixel)
...
def _rim_thickness_in_quadrant(
    ...
    # Distance transform on disc interior → distance to disc *boundary*
...
def classify_risk(
    ...
    Thresholds derived from clinical literature:
        vCDR < 0.65  → Healthy
        vCDR 0.65–0.80 + ISNT violation → Suspect
        vCDR > 0.80  → High Risk
    ...
    if vcdr > 0.80:
        risk = RiskLevel.HIGH
        warnings.append(f"vCDR {vcdr:.2f} exceeds 0.80 — urgent referral recommended.")
    elif vcdr > 0.65:
    ...
    if not isnt.rule_satisfied:
        warnings.append(
            "ISNT rule violated — neuro-retinal rim thinning detected."
        )
