...
#
# ESRGAN must score at least this much to be accepted.
# ------------------------------------------------------------
...
#
# If ESRGAN is worse than the original by more than this
# amount, it will be rejected.
# ------------------------------------------------------------
...
def prepare_pair(original, processed):
    ...
    We resize the processed image to the original dimensions
    only for evaluation. We do NOT modify the actual file.
    """
...
def calculate_sharpness(image, mask):
    """
    Measures fine detail using Laplacian variance.

    Higher normally means sharper.
    """
...
def calculate_contrast(image, mask):
    """
    Measures the spread of brightness values.
...
def calculate_noise(image, mask):
    ...
    This is deliberately conservative because aggressive
    sharpening can look like noise.
    """
...
def calculate_clipping(image, mask):
    """
    Calculates percentage of pixels that are almost black
    or almost white.

    Excessive clipping means loss of image information.
    """
...
def calculate_edge_density(image, mask):
    """
    Measures how much edge information exists.

    This is useful for detecting excessive sharpening.
    """
    ...
    return float(
        edge_pixels / total_pixels * 100.0
    )
...
def calculate_color_difference(original, processed, mask):
    ...
    difference = np.sqrt(
        np.sum(
            (
                original_lab.astype(np.float32)
                -
                processed_lab.astype(np.float32)
            ) ** 2,
            axis=2
        )
    )
...
def calculate_technical_score(metrics):
    """
    Converts raw measurements into a 0-100 technical score.

    The scoring is deliberately based on reasonable ranges
    rather than simply rewarding maximum sharpness.
    """
    ...
    # SHARPNESS
    #
    # Log scale prevents extremely sharp/noisy images from
    # dominating the score.
    # --------------------------------------------------------

    sharpness_score = np.clip(
        np.log1p(sharpness) / np.log1p(8000) * 100,
        0,
        100
    )
    ...
    # --------------------------------------------------------
    # CONTRAST
    # Good useful range approximately 25-80.
    # --------------------------------------------------------

    contrast_score = np.clip(
        100 - abs(contrast - 52) * 1.8,
        0,
        100
    )
    ...
    # BRIGHTNESS
    #
    # Avoid rewarding extremely dark or extremely bright images.
    # --------------------------------------------------------

    brightness_score = np.clip(
        100 - abs(brightness - 105) * 1.2,
        0,
        100
    )
    ...
    # EDGE DENSITY
    #
    # Very high edge density can indicate artificial
    # sharpening.
    # --------------------------------------------------------
    ...
    #
    # Sharpness is important but NOT dominant.
    # --------------------------------------------------------
...
def calculate_enhancement_safety(
    original_metrics,
    processed_metrics,
    ssim_score,
    color_difference
    ...
    """
    Determines whether ESRGAN improved the image without
    making suspiciously large changes.

    This is one of the most important parts of the evaluator.
    """

    # --------------------------------------------------------
    # 1. Noise change
    #
    # Large noise increase is bad.
    # --------------------------------------------------------
    ...
    # 2. Edge increase
    #
    # Moderate increase is okay.
    # Huge increase can indicate artificial sharpening.
    # --------------------------------------------------------
    ...
    #
    # Small color changes are acceptable.
    # Large changes are suspicious.
    # --------------------------------------------------------

    color_score = np.clip(
        100 - color_difference * 2.0,
        0,
        100
    )
...
def get_images(folder):
    """
    Returns all supported image files in a folder.

    red_flagged is deliberately ignored.
    """
...
def process_pair(
    ...
    original, processed = prepare_pair(
        original,
        processed
    )
    ...
    processed_metrics = evaluate_image(
        processed
    )
    ...
    mask = get_fundus_mask(
        original
    )

    color_difference = calculate_color_difference(
        original,
        processed,
        mask
    )
    ...
    safety_score = calculate_enhancement_safety(
        original_metrics,
        processed_metrics,
        ssim_score,
        color_difference
    ...
    improvement = (
        processed_metrics["technical_score"]
        -
        original_metrics["technical_score"]
    )
    ...
    final_score = (
        processed_metrics["technical_score"] * 0.40
        +
        safety_score * 0.60
    )
    ...
    if safety_score < 45:
        reasons.append(
            "STRUCTURAL_OR_ARTIFACT_RISK"
        )

    # Large noise increase.
    if (
        processed_metrics["noise"]
        >
        original_metrics["noise"] * 1.75
    ):
        reasons.append(
            "NOISE_INCREASE"
        )
    ...
    # Large edge increase.
    if (
        processed_metrics["edge_density"]
        >
        original_metrics["edge_density"] * 2.0
    ):
        reasons.append(
            "EXCESSIVE_EDGE_INCREASE"
        )
...
def main():
    ...
    if not original_folder.exists():
        raise FileNotFoundError(
            f"Original folder does not exist:\n"
            f"{original_folder}"
        )

    if not esrgan_folder.exists():
        raise FileNotFoundError(
            f"ESRGAN folder does not exist:\n"
            f"{esrgan_folder}"
        )
    ...
    original_dict = build_file_dictionary(
        original_images
    )

    esrgan_dict = build_file_dictionary(
        esrgan_images
    )
    ...
    print(
        f"Missing original      : "
        f"{len(missing_original)}"
    )

    print(
        f"Missing ESRGAN        : "
        f"{len(missing_esrgan)}"
    )
    ...
    if not common_names:
        ...
        print(
            "\nThe filenames must match between "
            "the two folders."
        )
    ...
    esrgan_dataframe = pd.DataFrame(
        esrgan_results
    )

    esrgan_dataframe.to_csv(
        esrgan_csv,
        index=False
    )
