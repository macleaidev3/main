...
from ml_module.ICE126.predict_density import DensityPredictor
from ml_module.ICE126.predict_total_pressure import TotalPressurePredictor
from ml_module.ICE126.predict_total_temperature import TotalTemperaturePredictor
from ml_module.ICE126.predict_h2s import H2SPredictor
from ml_module.ICE126.predict_hplus import HPlusPredictor
from ml_module.ICE126.predict_wall_shear import WallShearPredictor
from ml_module.ICE126.predict_turb_kinetic import TurbKineticEnergyPredictor
from ml_module.ICE126.predict_corrosion_rate import CorrosionRatePredictor


from src.ml_instrument.ICE126.input_df_builder import PredictionInputBuilder
from src.utils.core_utility_functions import sigfig, format_date_long
class CRPredictionICE126():
    ...
    def __init__(self, month: str, year: str, yesterday_date: str):
        ...
        self.year = year
        self.month = month
        self.table_name = f"{year}_126_cr"
    ...
    def set_up(self) -> None:
        ...
        if input_df.isna().any().any():
            ...
            target_density_column, pred_density = density_predictor.predict(input_df)
            input_df[target_density_column] = pred_density
            ...
            target_h2s_column, pred_h2s = h2s_predictor.predict(input_df)
            input_df[target_h2s_column] = pred_h2s

            hplus_predictor = HPlusPredictor()
            target_hplus_column, pred_hplus = hplus_predictor.predict(input_df)
            input_df[target_hplus_column] = pred_hplus
            ...
            target_turb_kinetic_column, pred_turb_kinetic = turb_kinetic_predictor.predict(input_df)
            input_df[target_turb_kinetic_column] = pred_turb_kinetic
