...
# from src.ip21.sync_from_client_db import IP21Synchronizer

class CreateIp21Window(QtWidgets.QWidget):
    _active_threads = set()
    @classmethod
    ...
    def __init__(self, parent=None):
        ...
        self.column_names = extract_column_names(TABLE_COLUMNS["ip21_data"])

        self.tab = CreateTabWidget(self)
        # # add the sync button
        # self.sync_button = QtWidgets.QToolButton(parent=self)
        # self.sync_button.setToolTip("Sync")
        # self.sync_button.setIcon(QtGui.QIcon(resource_path("assets/sync.png")))
        # self.sync_button.setIconSize(QtCore.QSize(23, 23))
        # self.sync_button.setAutoRaise(True)
        # self.sync_button.clicked.connect(self.sync_from_database)
        # self.tab.horizontalLayout.addWidget(self.sync_button)
    ...
    def set_up(self, year, month): # month in short format
        ...
        self.table_name = f"ip21_{year}_{month}"
        # self.table_name = "ip21_table"
    ...
    def month_changed(self):
        ...
        # print(f"Data fetched for year: {year}, month: {month}. Number of rows: {len(data)}")
    ...
    def load_initial_data(self):
        ...
        self.table.populate(rows) # Update your UI table
    ...
    def load_data(self):
        ...
        CreateIp21Window._active_threads.add(thread)
        ...
        thread.finished.connect(lambda: CreateIp21Window._cleanup_thread(thread))
    ...
    def _on_load_success(self):
        if self._loader_thread:
            ...
            try:
                dates = [datetime.strptime(row[0], "%d/%m/%y %H:%M").strftime("%d/%m/%y") for row in self.loaded_data]
            except ValueError:
                dates = [datetime.strptime(row[0], "%d-%m-%Y %H:%M").strftime("%d/%m/%Y") for row in self.loaded_data]
    ...
    def on_table_cell_changed(self, row, col, new_value):
        ...
        # get the time(to be used as key) from the first column of the edited row
        time_index = 0
        time_value = self.table._table_model._data[row][time_index]

        # get the whole row data as a list to be used as value in the dict
        row_data = list(self.table._table_model._data[row])

        # update on self._edited_row_dict with key as time_value and value as row_data
        self._edited_row_dict[time_value] = row_data
    ...
    def save(self):
        ...
        if self._data_just_loaded_from_csv: # if data is just loaded from csv
            ...
            is_valid, error_messages, cleaned_data = validate_and_clean_ui_data(self.loaded_data, TABLE_COLUMNS['ip21_data'])
            ...
            is_updated = self.db_manager.save(self.db_name, "Time", self.table_name, cleaned_data)
            ...
            is_valid, error_messages, cleaned_data = validate_and_clean_ui_data(
                row_list, TABLE_COLUMNS['ip21_data']
            )
            ...
            for time_value, row_data in self._edited_row_dict.items():
                temp_data_dict = {}
                for column_name in self.column_names:
                    temp_data_dict[column_name] = row_data[self.column_names.index(column_name)]

                self.db_manager.update_a_row(db_name=self.db_name, table=self.table_name, pk_column="Time", pk_value=time_value, data=temp_data_dict)
    ...
    #     """
    #     Triggered by self.sync_button. 
    #     Fetches client data, asks user for confirmation based on extracted dates, 
    #     validates, and saves to the software database.
    #     """
    #     # 1. Initialize the helper class
    #     synchronizer = IP21Synchronizer(self.db_manager, client_db_name="kr_db")

    #     try:
    #         # 2. Fetch the data and get the generated summary
    #         QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.CursorShape.WaitCursor)
    #         grouped_data, summary_text = synchronizer.fetch_and_group_data()
    #         QtWidgets.QApplication.restoreOverrideCursor()
            
    #         if not grouped_data:
    #             QtWidgets.QMessageBox.information(self, "Sync Information", "No valid data found in the client database to sync.")
    #             return

    #         # 3. Present the summary to the user and ask for confirmation
    #         confirmation_message = (
    #             "The following data dates were found in the client database:\n\n"
    #             f"{summary_text}\n\n"
    #             "Do you want to proceed with the synchronization?"
    #         )
    ...
    #         # 4. Iterate over the grouped target tables to Validate and Save
    #         QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.CursorShape.WaitCursor)
            
    #         for target_table_name, rows in grouped_data.items():
    ...
    #                     self, 
    #                     "Data Validation Error", 
    #                     f"Validation failed for {target_table_name}. Please fix the following errors:\n\n{error_text}"
    #                 )
    #                 return # Abort the rest of the sync if data is corrupt

    #             # Save using your database manager. 
    #             # Note: Assuming "Time" is the designated primary key for your software DB schema.
    #             is_updated = self.db_manager.save(
    #                 self.db_name, 
    #                 "Time", 
    #                 target_table_name, 
    #                 cleaned_data
    ...
    #                     self, 
    #                     "Database Error", 
    #                     f"An error occurred while saving data to table: {target_table_name}"
    #                 )
    #                 return

    #         # 5. Success
    #         QtWidgets.QApplication.restoreOverrideCursor()
    #         QtWidgets.QMessageBox.information(self, "Sync Complete", "All data was successfully synchronized!")

    #     except Exception as e:
    #         QtWidgets.QApplication.restoreOverrideCursor()
    #         QtWidgets.QMessageBox.critical(self, "Critical Error", f"An unexpected error occurred:\n{str(e)}")

    def sync_from_database(self):
        from src.ip21_sync.synchronizer import IP21Synchronizer
        from src.utils.table_columns import TABLE_COLUMNS

        sync = IP21Synchronizer(
