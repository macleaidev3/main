...
class DatabaseManager:
    ...
    def update_a_row(self, db_name: str, table: str, pk_column: str, pk_value: Any, data: Dict[str, Any]):
        """
        Updates multiple columns for a single row identified by its primary key.
        ...
        Args:
            ...
            data (Dict[str, Any]): Dictionary mapping logical column names to their new values.
        """
        safe_pk = self._sanitize_col_name(pk_column)
        cols_list = [f"[{self._sanitize_col_name(col)}] = ?" for col in data.keys()]
        
        cols = ", ".join(cols_list)
        values = list(data.values())
        values.append(pk_value)

        query = f"UPDATE [{table}] SET {cols} WHERE [{safe_pk}] = ?"
