...
class LSTMModel(nn.Module):
    def __init__(self, input_size):
        super().__init__()
        self.lstm = nn.LSTM(input_size, 32, num_layers=2, batch_first=True, dropout=0.1)
...
class ID00005Model:
    def __init__(
    self,
    artifact_dir=resource_path("ml_module/cache_mlmodule_1232232/ID_00005"),
    historical_path=resource_path("ml_module/cache_mlmodule_1232232/ID_00005/test_original.csv")
):
        """
        Initializes the model, loads scalers, and pre-loads the historical data into memory.
        """

        logger.info("Initializing ID00005 corrosion probe prediction model.")
        ...
        try:
            ...
            logger.info(
                "ID00005 corrosion probe prediction model initialized successfully."
            )

        except Exception:
            logger.exception(
                "Failed to initialize ID00005 corrosion probe prediction model."
            )
            raise
