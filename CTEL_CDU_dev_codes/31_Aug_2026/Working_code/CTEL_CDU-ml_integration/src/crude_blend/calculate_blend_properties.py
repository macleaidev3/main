...
class BlendPropertiesCalculation:
    ...
    def update_blend_properties(self, given_date: str) -> None:
        ...
        # ----------------------------------------------------------
        # Determine blend composition
        # ----------------------------------------------------------

        try:
            column_name = format_date_long(given_date)

            total_crude_volume = self.db_manager.get_cell_value(
                db_name=self.db_name,
                table_name=blend_table,
                target_column=column_name,
                pk_column="crude_name",
                pk_value="TOTAL CRUDE",
            )

        except Exception:
            logger.exception(
                "[BlendProperties] Failed to retrieve total crude volume."
            )
            return
        ...
        if total_crude_volume is None:
            logger.warning(
                "[BlendProperties] Total crude volume is NULL for %s",
                given_date,
            )
            return

        if total_crude_volume <= 0:
            logger.info(
                "[BlendProperties] Total crude volume is %.3f. "
                "No blend calculation required.",
                total_crude_volume,
            )

            try:
                rows = self.db_manager.read_table(
                    self.db_name,
                    self.table_name,
                )

                logger.debug(
                    "[BlendProperties] Existing blend property rows: %s",
                    rows,
                )

            except Exception:
                logger.exception(
                    "[BlendProperties] Unable to read blend property table."
                )

            return

        logger.info(
            "[BlendProperties] Total crude volume = %.3f",
            total_crude_volume,
        )
        ...
        for crude_name, volume in column_data:
            ...
            if crude_name in self.IGNORED_ROWS:
                continue
