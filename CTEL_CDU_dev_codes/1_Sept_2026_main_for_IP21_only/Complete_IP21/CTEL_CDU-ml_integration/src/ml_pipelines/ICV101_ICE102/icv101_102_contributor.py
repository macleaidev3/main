...
class CRContributorICV101toICE102():
    
    def __init__(self, month: str, year: str, yesterday_date: str):
        """
        Initializes the CRContributorICV101toICE102 class.

        Args:
            yesterday_date (str): The date for which data needs to be fetched. dd/mm/yyyy
        """
        logger.debug("Initializing CRContributorICV101toICE102 for Date: %s", yesterday_date)
    ...
    def set_up(self) -> None:
        ...
        if not is_updated:
            logger.warning("CRContributorICV101toICE102 Aborted: Daily lab report data for '%s' is not updated for %s.", table, self.yesterday_date)
            return
        ...
        if not is_updated:
            logger.warning("CRContributorICV101toICE102 Aborted: IP21 data for '%s' is not updated for %s.", table, self.yesterday_date)
            return
        ...
        print(f"Data to be updated from icv101_ice102 contributor thread: {self.data_to_be_updated}")
        table_name = f"{self.year}_{self.month}_101_to_102_contributor"
        ...
        logger.info("CRContributorICV101toICE102 completed for Date: %s", self.yesterday_date)
