...
training, predicts Wall Shear, and returns the prediction without modifying
the input DataFrame.
"""
...
MODEL_PATH = resource_path(
    "ml_module/cache_mlmodule_1232232/ICV113/wall_shear.joblib"
)


class WallShearPredictor:
    """
    Predictor for Wall Shear.
    """
    ...
    def _validate_features(self, df: pd.DataFrame) -> None:
        ...
        if missing:
            raise ValueError(
                "Missing input features for Wall Shear prediction: "
                f"{missing}"
            )