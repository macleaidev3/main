...
def year_month_days_list_of_dict(year: int) -> List[Dict[int, Dict[str, List[str]]]]:
        ...
        Month names follow the requested notation: "Jan", "Feb", "March", "April", "May", "June",
...
def get_days_in_month(year: int, month: int) -> list[str]:
        ...
        days_list = [datetime.date(year, month, day).strftime("%d/%m/%Y") for day in range(1, num_days + 1)]
        return days_list
...
def build_blend_ordered_row():
    ...
    def _classify_crudes(data):
        ...
        sulphur_type must be one of: "Low sulphur", "Medium sulphur", "High sulphur"
        origin = "India" → goes to indigenous regardless of sulphur type
        """ 

        classified = { "indigenous": [], "imported_ls": [], "imported_ms": [], "imported_hs": [] }
    ...
    def build_blend_table_rows(indigenous, imported_ls, imported_ms, imported_hs):
        """
    ...
    ordered_row_data = build_blend_table_rows(indigenous, imported_ls, imported_ms, imported_hs)
...
def get_global_min_max_cr():
    ...
    from src.server_manager.operation_manager import DatabaseManager
    db_manager = DatabaseManager()
    result = db_manager.read_columns("SentinelDB", "global_min_max_cr", ["global_min", "global_max"])
    if result:
...
def update_global_min_max_cr(global_min, global_max):
    ...
    db_manager.update_a_row("SentinelDB", "global_min_max_cr", "primary_key", 1,
...
def check_daily_lab_report_data_update(date: str):  ### me
    """
    Check whether the daily Lab Report date exists in all
    six Lab Report sections.
    IMPORTANT:
        This function only checks the database state.
        Missing Lab Report dates are recovered BEFORE this
        function is called by UTThicknessContributor.
    Therefore this function remains a validation/check function
    and does not perform averaging itself.
    """
    ...
    # --------------------- ALL SIX LAB REPORT SECTIONS-----------------------------------------------------
    table_name_list = [ "after_desalter_stage_1", "after_desalter_stage_2", "crude_before_desalter",
        "sour_water_icv112", "sour_water_icv113", "stripped_water",
    ]

    for base_table_name in table_name_list:
        print( f"Checking table: {base_table_name}..." )
        table_name = ( f"lab_{year}_{month_short}_{base_table_name}" )
        ...
        if date_list:
            for row in date_list:
                ...
                for fmt in ("%d/%m/%Y %H:%M", "%d/%m/%Y %H:%M:%S", "%d-%m-%Y %H:%M", "%d-%m-%Y %H:%M:%S",
                    "%d/%m/%y %H:%M", "%d/%m/%y %H:%M:%S", "%d-%m-%y %H:%M", "%d-%m-%y %H:%M:%S", "%d/%m/%Y",
                    "%d/%m/%y", "%d-%m-%Y", "%d-%m-%y",
                ):
                    try:
                ...
                if parsed is None:
                    try:
                        ...
                        raise ValueError(f"Invalid Lab Report date format: {text}")
                updated_date_list.append(parsed.strftime("%d/%m/%Y"))

        if date not in updated_date_list:
