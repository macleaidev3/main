...
class CRContributorICE162():
    
    def __init__(self, month: str, year: str, yesterday_date: str):
        """
        Initializes the CRContributorICE162 class.

        Args:
            yesterday_date (str): The date for which data needs to be fetched. dd/mm/yyyy
        """
        logger.debug("Initializing CRContributorICE162 for Date: %s", yesterday_date)
    ...
    def set_up(self) -> None:
        ...
        if not is_updated:
            logger.warning("CRContributorICE162 Aborted: Daily lab report data for '%s' is not updated for %s.", table, self.yesterday_date)
            return
        ...
        if not is_updated:
            logger.warning("CRContributorICE162 Aborted: IP21 data for '%s' is not updated for %s.", table, self.yesterday_date)
            return
        ...
        # name in blend properties table, name in contributor table
        ...
        print(f"Data to be updated from ice162 contributor thread: {self.data_to_be_updated}")
        table_name = f"{self.year}_{self.month}_162_contributor"
        ...
        logger.info("CRContributorICE162 completed for Date: %s", self.yesterday_date)
