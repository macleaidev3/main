...
from ml_module.ICE162.predict_wall_shear import WallShearPredictor
from ml_module.ICE162.predict_total_temp import TotalTemperaturePredictor
from ml_module.ICE162.predict_total_pressure import TotalPressurePredictor
from ml_module.ICE162.predict_cr_rate import CorrosionRatePredictor



from src.ml_instrument.ICE162.input_df_builder import PredictionInputBuilder
from src.utils.core_utility_functions import sigfig, format_date_long
class CRPredictionICE162():
    ...
    def __init__(self, month: str, year: str, yesterday_date: str):
        ...
        self.year = year
        self.month = month
        self.table_name = f"{year}_162_cr"
    ...
    def set_up(self) -> None:
        ...
        if input_df.isna().any().any():
            ...
            total_temp_predictor = TotalTemperaturePredictor()
            target_total_temp_column, predicted_total_temp = total_temp_predictor.predict(input_df)
            input_df[target_total_temp_column] = predicted_total_temp

            total_pressure_predictor = TotalPressurePredictor()
            target_total_pressure_column, predicted_total_pressure = total_pressure_predictor.predict(input_df)
            input_df[target_total_pressure_column] = predicted_total_pressure
