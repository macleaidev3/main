...
class FastTableModel(QtCore.QAbstractTableModel):
    ...
    def add_row(self, row_data=None):
        ...
        if row_data is None:
            ...
            # Pad with empty strings if the provided
            # data is shorter than column count
            while len(new_row) < len(self._columns):
                new_row.append("")

            # Truncate if the provided data is longer
            # than column count
            new_row = new_row[
                :len(self._columns)
            ]
...
class FrozenTable(QtWidgets.QWidget):
    ...
    # ============================================================
    # NEW:
    # Resize rows for multi-line cell contents
    # ============================================================
    ...
    def resize_rows_to_contents(self):
        ...
        This is especially important for the Flag column,
        ...
        main_header = (
            self._main_view.verticalHeader()
        )

        frozen_header = (
            self._frozen_view.verticalHeader()
        )

        # --------------------------------------------------------
        # Temporarily block row-height synchronization signals.
        #
        # This prevents the two views from repeatedly resizing
        # each other.
        # --------------------------------------------------------
        ...
        try:
            ...
            # Let Qt calculate the required row height based
            # on wrapped text.
            ...
            for row in range(
                ...
                height = (
                    main_header.sectionSize(row)
                )

                frozen_header.resizeSection(
                    row,
                    height
                )
    ...
    def _create_views(self):
        ...
        # Create delegate
        self.edit_delegate = EditTrackingDelegate(
            self
        )
        ...
        self._main_view = QtWidgets.QTableView(
            self
        )

        self._main_view.setModel(
            self._model
        )

        self._main_view.setItemDelegate(
            self.edit_delegate
        )

        self._main_view.setAlternatingRowColors(
            True
        )
        ...
        self._main_view.setWordWrap(
            True
        )

        self._main_view.setTextElideMode(
            QtCore.Qt.TextElideMode.ElideNone
        )

        self._main_view.setSelectionBehavior(
        ...
        main_header = (
            self._main_view.horizontalHeader()
        )

        main_header.setSectionResizeMode(
            QtWidgets.QHeaderView.ResizeMode.Interactive
        )

        main_header.setMinimumSectionSize(
        ...
        self._frozen_view = QtWidgets.QTableView(
            self
        )

        self._frozen_view.setModel(
            self._model
        )

        self._frozen_view.setItemDelegate(
            self.edit_delegate
        )

        self._frozen_view.setAlternatingRowColors(
            True
        )
        ...
        self._frozen_view.setWordWrap(
            True
        )

        self._frozen_view.setTextElideMode(
            QtCore.Qt.TextElideMode.ElideNone
        )
        ...
        frozen_header = (
            self._frozen_view.horizontalHeader()
        )
        ...
        frozen_header.setMaximumSectionSize(
        ...
        for col in range(
            ...
            if col < self.frozen_columns:
                ...
                self._main_view.hideColumn(
                    col
                )

            else:

                self._frozen_view.hideColumn(
                    col
                )
        ...
        self._main_view.verticalHeader().setVisible(
            False
        )

        self._frozen_view.verticalHeader().setVisible(
            True
        )
        ...
        self._main_view.setSelectionModel(
            selection_model
        )

        self._frozen_view.setSelectionModel(
            selection_model
        )
    ...
    def _connect_signals(self):
        ...
        main_vbar = (
            self._main_view.verticalScrollBar()
        )

        frozen_vbar = (
            self._frozen_view.verticalScrollBar()
        )

        frozen_vbar.hide()

        main_vbar.valueChanged.connect(
            frozen_vbar.setValue
        )

        frozen_vbar.valueChanged.connect(
            main_vbar.setValue
        )
        ...
        # self.edit_delegate.editingStarted.connect(
        #     self._on_edit_start
        # )

        # self.edit_delegate.editingFinished.connect(
        #     self._on_edit_end
        # )
    ...
    def _resize_frozen_area(self):
        ...
        width = (
            self._frozen_view.verticalHeader().width()
        )
        ...
        self._frozen_view.setFixedWidth(
            width + 2
        )
    ...
    def add_row(self, row_data=None):
        ...
        # Recalculate frozen area in case the vertical
        # header width increased.
        self._resize_frozen_area()

        # --------------------------------------------------------
        # NEW:
        # Resize row if the newly added row contains
        # multi-line text.
        # --------------------------------------------------------
        self.resize_rows_to_contents()
    ...
    def clear_table(self):
        """
        Clears all rows from the table and resets
        the frozen area.
        """

        self._model.clear_data()

        # After clearing, the vertical header width
        # might shrink.
        self._resize_frozen_area()
