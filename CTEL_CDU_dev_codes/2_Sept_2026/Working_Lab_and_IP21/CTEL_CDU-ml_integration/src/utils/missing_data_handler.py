...
class MissingDataHandler:
    ...
    A missing value is calculated from the previous
    ...
    def _ensure_flag_column(self, dataframe):
        ...
        if self.FLAGS_COLUMN not in dataframe.columns:
            dataframe[self.FLAGS_COLUMN] = pd.Series(
                np.nan,
                index=dataframe.index,
                dtype="object",
            )
        elif not pd.api.types.is_object_dtype(
            dataframe[self.FLAGS_COLUMN]
        ):
            dataframe[self.FLAGS_COLUMN] = (
                dataframe[self.FLAGS_COLUMN].astype("object")
            )
    ...
    def _prepare_dataframe(self, dataframe, time_column):
        ...
        dataframe[time_column] = dataframe[
            time_column
        ].map(self._normalize_timestamp)
    ...
    def _set_source_boundaries(self, dataframe, time_column):
        ...
        self._minimum_available_timestamp = dataframe[
            time_column
        ].min()

        self._maximum_available_timestamp = dataframe[
            time_column
        ].max()
    ...
    def _find_row(cls, dataframe, timestamp, time_column):
        ...
        matches = dataframe.index[
            dataframe[time_column] == timestamp
        ].tolist()
    ...
    def _validate_source_history(
        ...
        oldest, latest = self._required_range(
            target_timestamp
        )
    ...
    def _get_historical_timestamps(self, target_timestamp):
        ...
        Example:
            ...
            target - 720 hours
        """
        target_timestamp = self._normalize_timestamp(
            target_timestamp
        )
    ...
    def _recover_historical_value(
        ...
        """
        Return an existing historical value or recover it
        recursively when missing.
        """
        value = self._get_existing_value(
            dataframe,
            timestamp,
            column,
            time_column,
        ...
        logger.debug(
            "[MissingData] Historical value missing | "
            "Timestamp=%s | Column=%s | Recursive recovery",
            timestamp,
            column,
        )

        return self._fill_one_value(
            dataframe,
            timestamp,
            column,
            time_column,
    ...
    def _collect_historical_values(
        ...
        for timestamp in self._get_historical_timestamps(
            target_timestamp
        ):
            value = self._recover_historical_value(
                dataframe,
                timestamp,
                column,
                time_column,
            ...
            if not self._is_valid_value(value):
                raise ValueError(
                    "Recursive recovery returned an invalid "
                    f"value for {timestamp} / {column}."
                )
        ...
        if len(values) != self.LOOKBACK_HOURS:
            raise ValueError(
                f"Expected {self.LOOKBACK_HOURS} historical "
                f"values for {target_timestamp} / {column}; "
                f"obtained {len(values)}."
            )
    ...
    def _calculate_missing_value(
        ...
        values = self._collect_historical_values(
            dataframe,
            target_timestamp,
            column,
            time_column,
    ...
    def _store_value(
        ...
        A valid existing value is always preserved.
        """
        row_index = self._find_row(
            dataframe,
            timestamp,
    ...
    def _get_flag_message(cls, timestamp):
        ...
        date_text = pd.Timestamp(timestamp).strftime(
            "%d/%m/%Y"
        )

        return (
            f"{date_text} IP21 data was not available, "
    ...
    def _add_flag(
        ...
        dataframe.at[
            row_index,
            self.FLAGS_COLUMN,
        ] = self._get_flag_message(timestamp)
    ...
    def _record_fill(self, timestamp, column):
        ...
        timestamp_text = pd.Timestamp(timestamp).strftime(
            "%d-%m-%Y %H:%M"
        )
    ...
    def _log_source_range(self, dataframe):
        ...
        if (
            self._minimum_available_timestamp is None
    ...
    def _fill_one_value(
        ...
        """
        Recover one missing hourly value.

        The calculation uses exactly the previous 720
        ...
        dependency_key = self._begin_dependency(
            target_timestamp,
            column,
        )

        try:
            average_value = self._calculate_missing_value(
                dataframe,
                target_timestamp,
                column,
                time_column,
    ...
    def fill_missing_data(
        ...
        For every missing value, the handler uses:
