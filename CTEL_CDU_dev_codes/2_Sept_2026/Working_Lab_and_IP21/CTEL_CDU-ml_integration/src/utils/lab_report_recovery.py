...
class LabReportRecoveryManager:
    ...
    def _table_name(
        ...
        return (
            f"lab_{target_date.year}_"
            f"{month}_{section_key}"
        )
    ...
    def _load_section_data(
        ...
        Time is ignored and all rows are normalized to one
        date.

        A broad historical window is loaded so that missing
        historical values can be recursively recovered.
        ...
        while current_month <= end_month:
            ...
            try:
                ...
                rows = self.db_manager.read_columns(
                    self.DB_NAME,
                    table_name,
                    columns,
                )
                ...
                # A month table may legitimately not exist.
                # Treat that month as having no Lab data.
                logger.debug(
                    "[LAB] Could not read table %s",
                    table_name,
                    exc_info=True,
                )
            ...
            for row in rows:
                ...
                # One row = one date.
                #
                # If duplicate rows exist for the same date,
                # preserve already valid values and use another
                # non-empty value only where required.
                # ------------------------------------------------
    ...
    def _calculate_previous_30_day_average(
        ...
        Rules:
            ...
            1. Valid existing values are used directly.
            2. If a historical date is missing, that date is
               recursively recovered from its own previous
               30 calendar days.
            3. Recovered historical values are used only in
               memory.
            4. If the required historical chain reaches the
               beginning of available historical information,
               recovery stops and the value remains blank.
            5. A circular dependency can never cause infinite
               recursion.
            6. All 30 previous calendar-day values are required.
        """
        ...
        try:
            ...
            for day_offset in range(
                ...
                # ------------------------------------------------
                # If the requested dependency lies outside the
                # loaded historical window, it cannot be safely
                # recovered from the current dataset.
                #
                # Do NOT create endless empty records.
                # ------------------------------------------------
                ...
                #
                # Create only an in-memory placeholder.
                # ------------------------------------------------
                ...
                #
                # Recover it recursively before using it.
                # ------------------------------------------------
                ...
                # ------------------------------------------------
                # Store recovered value only in memory.
                # ------------------------------------------------

                record[
                    "values"
                ][column] = recovered_value
            ...
            # ----------------------------------------------------
            # The task requires the previous 30 calendar days.
            #
            # Therefore, do not calculate an average from fewer
            # than 30 values.
            # ----------------------------------------------------
    ...
    def _recover_section(
        ...
        display_name = self.SECTION_CONFIG[
            section_key
        ]

        logger.info(
            "[LAB] Processing section: %s | Date=%s",
            display_name,
            self._format_date(
                target_date
            ),
        ...
        records = self._load_section_data(
            section_key,
            target_date,
        )
        ...
        # Situation 3:
        #
        # Entire target date does not exist.
        #
        ...
        for column in feature_columns:
            ...
            # ----------------------------------------------------
            # Situation 1 / valid uploaded value:
            # NEVER overwrite it.
            # ----------------------------------------------------
            ...
            if recovered_value is None:
                ...
                logger.warning(
                    "[LAB] Could not recover | "
                    "Section=%s | Date=%s | Column=%s | "
                    "Leaving blank.",
                    display_name,
                    self._format_date(
                        target_date
            ...
            recovered_target_columns.append(
                column
            )

            logger.info(
                "[LAB] Target value recovered | "
                "Section=%s | Date=%s | Column=%s | "
                "Average=%s",
                display_name,
                self._format_date(
        ...
        # --------------------------------------------------------
        # Persist ONLY the target date.
        #
        # Recursively recovered historical values remain
        # in-memory and are NEVER written into old Lab records.
        # --------------------------------------------------------
        ...
        if target_date_was_missing:
            ...
            for column_name, _ in column_definitions:
                ...
                if column_name == "Date":
                    ...
                    # Text fields such as Remark remain blank.
                    full_row.append(
                        values.get(
                            column_name
                        )
            ...
            # Existing target row.
            #
            # Update ONLY the missing/recovered feature columns.
            # ----------------------------------------------------
            ...
            try:
                ...
                logger.info(
                    "[LAB] Updated missing target "
                    "feature values | Section=%s | "
                    "Date=%s | Columns=%s",
                    display_name,
                    self._format_date(
                        target_date
    ...
    def recover_for_date(
        ...
        if target_date is None:
            ...
            raise ValueError(
                f"Invalid Lab Report prediction date: "
                f"{date_str}"
            )
        ...
        logger.info(
            "[LAB] Starting Lab Report recovery | Date=%s",
            self._format_date(
                target_date
            ),
        )
        ...
        if len(
            ...
            section_text = (
                f'section "{recovered_sections[0]}"'
            )
        ...
        message = (
            f"{self._format_date(target_date)} "
            "Lab Report data was not available in "
            f"{section_text}, so Sentinal has averaged "
            "the data of the last 30 days to predict "
            "the Cr/Thickness"
        )

        logger.warning(
            "[LAB] FLAG GENERATED | %s",
            message,
        )
