...
class FastTableModel(QtCore.QAbstractTableModel):
    ...
    def add_row(self, row_data=None):
        ...
        if row_data is None:
            ...
            new_row = new_row[:len(self._columns)]

        # 2. Insert the row safely using Qt Model/View signals
        row_index = len(self._data) # Index where the new row will be placed (the end)
...
class FrozenTable(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        self.frozen_columns = max(0, min(frozen_columns, len(self.column_names)))
    ...
    def _connect_signals(self):
        ...
        self._main_view.verticalHeader().sectionResized.connect(
            lambda i, o, n: self._frozen_view.verticalHeader().resizeSection(i, n)
        )
        self._frozen_view.verticalHeader().sectionResized.connect(
            lambda i, o, n: self._main_view.verticalHeader().resizeSection(i, n)
        )
    ...
    def _resize_frozen_area(self):
        ...
        for col in range(self.frozen_columns):
            width += self._frozen_view.columnWidth(col)
    ...
    def _adjust_column_widths(self):
        ...
        for col, name in enumerate(self.column_names):
            w = fm.horizontalAdvance(str(name)) + 20
            w = max(self.min_column_width, min(w, self.max_column_width))
            self._main_view.setColumnWidth(col, w)
    ...
    def add_row(self, row_data=None):
        ...
        row_data: Optional tuple or list containing the row values.
        """
        self._model.add_row(row_data)
        
        # Recalculate frozen area in case the vertical header width increased 
        # (e.g., row count goes from 9 to 10, requiring more pixel width for the number)
        self._resize_frozen_area()
    ...
    def clear_table(self):
        ...
        # (e.g., from row 1000 back to nothing), so we refresh the layout.
        self._resize_frozen_area()
    ...
    def update_table(self, data):
        ...
        """
        # Simply update the model - it handles the reset signals internally
        self._model.update_all_data(data)
        
        # Crucial: Recalculate widths and frozen area for the new data
        self._adjust_column_widths()
...
if __name__ == "__main__":
    ...
    rows = [(i, i * 2, i * 3, i * 4, i * 5) for i in range(200000)]
