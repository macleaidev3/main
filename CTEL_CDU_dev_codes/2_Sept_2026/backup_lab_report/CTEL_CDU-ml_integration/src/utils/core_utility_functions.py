...
def check_daily_lab_report_data_update(date: str):  ### me
    ...
    print(
        f"Checking daily lab report data update "
        f"for date: {date}..."
    )
    ...
    for base_table_name in table_name_list:
        ...
        print(
            f"Checking table: {base_table_name}..."
        )

        table_name = (
            f"lab_{year}_{month_short}_{base_table_name}"
        )

        date_list = db_manager.read_columns(
            db_name,
            table_name,
            ["Date"]
        )
