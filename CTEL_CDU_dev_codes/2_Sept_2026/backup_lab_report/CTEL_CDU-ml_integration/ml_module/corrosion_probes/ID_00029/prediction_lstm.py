...
class LSTMModel(nn.Module):
    def __init__(self, input_size):
        super().__init__()
        self.lstm = nn.LSTM(input_size, 16, num_layers=1, batch_first=True, dropout=0.3)
...
class ID00029Model:
    def __init__(
        self,
        artifact_dir=resource_path("ml_module/cache_mlmodule_1232232/ID_00029"),
        historical_path=resource_path("ml_module/cache_mlmodule_1232232/ID_00029/test_original.csv")
    ):
        """
        Initializes the model, loads scalers, and pre-loads the historical data into memory.
        """

        logger.info("Initializing ID00029 corrosion probe prediction model.")
        ...
        try:
            ...
            logger.info(
                "ID00029 corrosion probe prediction model initialized successfully."
            )

        except Exception:
            logger.exception(
                "Failed to initialize ID00029 corrosion probe prediction model."
            )
            raise
