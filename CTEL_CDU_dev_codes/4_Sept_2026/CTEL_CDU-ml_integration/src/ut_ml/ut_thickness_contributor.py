...
class UTThicknessContributor:
    ...
    def __init__(
        ...
        self.lab_report_recovery = LabReportRecoveryManager(db_manager=self.db_manager)
    ...
    def _get_target_datetime(self):
        ...
        target = pd.to_datetime(self.yesterday_date,dayfirst=True,errors="coerce")

        if pd.isna(target):
            raise ValueError(f"Invalid prediction date: {self.yesterday_date}")
        return self._normalize_timestamp(target).replace(hour=0)
    ...
    def _get_ip21_historical_data(self, target_datetime):
        ...
        """
        return self.ip21_recovery.get_historical_data(target_datetime= target_datetime)

    # ========================ORIGINAL TARGET STATE ========================

    def _determine_original_state(self,dataframe,target_datetime):
        """
        Determine whether the original target date had complete
        24-hour IP21 data.
        """
        state = self.ip21_recovery.determine_original_state(dataframe=dataframe, target_datetime=target_datetime)
        (
    ...
    def _prepare_target_ip21_data(self, dataframe, target_datetime):
        """
        ...
        result = self.ip21_recovery.prepare_target_data(dataframe=dataframe, target_datetime=target_datetime)
    ...
    def _get_ip21_table_name(self, target_datetime):
        ...
        Example:
            ...
            -> ip21_2025_Nov
        """
        month_name = month_short_name()[target_datetime.month - 1]
        return (f"ip21_{target_datetime.year}_{month_name}")
    ...
    def _save_recovered_ip21_data(self, target_datetime):
        ...
        if not self.data_was_filled_using_30_day_average:
            logger.info("[IP21] No recovered values require database synchronization.")
            return
        if self.ip21_data is None:
            raise RuntimeError("Cannot save recovered IP21 data because self.ip21_data is None.")
        if not self.filled_ip21_values:
            logger.warning("[IP21] Recovery was reported as used, but no recovered target values were recorded.")
            return

        table_name = self._get_ip21_table_name(target_datetime)
        logger.info("[IP21] Saving recovered target-date data to database table: %s", table_name)
        saved_rows = 0

        # -----------------------Process only target timestamps that were actually recovered. -----------------------
        for timestamp_text, recovered_columns in (self.filled_ip21_values.items()):
            ...
            if pd.isna(timestamp):
                logger.error("[IP21] Invalid recovered timestamp: %s",timestamp_text)
                continue

            timestamp = self._normalize_timestamp(timestamp)

            # ---------------------Safety check: Only write target-date rows. -------------------------
            if timestamp.date() != target_datetime.date():
                logger.warning("[IP21] Skipping non-target recovered timestamp: %s", timestamp_text)
                continue
            ...
            if not target_rows:
                logger.error("[IP21] Recovered timestamp %s was not found in self.ip21_data.", timestamp_text)
                continue
            row_index = target_rows[0]

            # ----------------------Build only the recovered values.Time is the primary key.--------------------------------

            recovered_data = {}
            for column in recovered_columns:
                if column not in self.ip21_data.columns:
                    logger.warning("[IP21] Recovered column '%s' is not present in DataFrame.", column)
                    continue

                value = self.ip21_data.at[row_index,column]
                if not self._is_valid_number(value):
                    logger.warning("[IP21] Skipping invalid recovered value | Time=%s | Column=%s | Value=%s",
                        timestamp_text,
                        column,
                        value,
                    )
            ...
            if not recovered_data:
                logger.warning("[IP21] No valid recovered values to save for %s.", timestamp_text)
                continue

            # ---------------- Database Time format.
            # Existing IP21 tables store Time values such as:
            #     26-11-2025 00:00 ----------------------------------------------------
            database_timestamp = timestamp.strftime("%d-%m-%Y %H:%M")

            # --------------------------update_a_row() performs UPDATE when the timestamp exists and INSERT when it does not.-----------------------------------------
            self.db_manager.update_a_row( self.db_name, table_name, self.TIME_COLUMN,
                database_timestamp,
                recovered_data,
            )
            saved_rows += 1
            logger.info("[IP21] RECOVERED DATA SAVED | Time=%s | Columns=%s",database_timestamp,
                list(recovered_data.keys()),
            )
        logger.info("[IP21] Database recovery synchronization completed | Table=%s | Rows processed=%d", table_name, saved_rows)
    ...
    def _create_ip21_flag(self):
        ...
        if not ip21_recovery_required:
            self.flags = None
            logger.info("[IP21] No missing-data recovery occurred. No IP21 flag generated.")
            return

        # ------------------- Preserve the existing IP21 flag message exactly. --------------------

        self.flags = (f"{self.yesterday_date} IP21 data was not available, so Sentinal has averaged the data of the last 30 days to predict the Cr/Thickness")
        logger.warning("[IP21] FLAG GENERATED | %s",self.flags)

        # --------------- Apply the flag to the in-memory IP21 DataFrame. --------------------
        if self.ip21_data is None:
            logger.warning("[IP21] Cannot apply flag because self.ip21_data is None.")
            return
        ...
        if target_row_count == 0:
            logger.warning("[IP21] Flag could not be applied because no target-date rows were found for %s.", self.yesterday_date)
            return
        ...
        logger.info("[IP21] Flag applied to %d target-date rows.",target_row_count)
    ...
    def _create_combined_flag(self):
        ...
        # --------------------IP21 flag ---------------------
        if self.flags:
            messages.append(str(self.flags).strip())

        # ---------------------Lab Report flag ---------------------
        if self.lab_flag:
            messages.append(str(self.lab_flag).strip())

        # ----------------------No recovery from either source. ----------------------
        if not messages:
            self.combined_flag = None
            logger.info("[FLAG] No IP21 or Lab Report recovery flag.")
            return
        ...
        logger.warning("[FLAG] FINAL COMBINED FLAG | Date=%s | %s", self.yesterday_date, self.combined_flag)
    ...
    def _write_flag_to_thickness_table(self):
        ...
        if not self.combined_flag:
            logger.info("[FLAG] No flag to write for %s.",self.yesterday_date)
            return
        ...
        database_date = target_datetime.strftime("%d/%m/%Y")

        # ----------------------Thickness table. -------------------------------
        table_name = (f"ut_{self.probe_id}_{self.year}_{self.month}_thickness")
        logger.warning("[FLAG] Attempting FINAL flag write | DB=%s | Table=%s | Date=%s | Flag=%s",
            self.db_name, table_name,  database_date, self.combined_flag
        )

        # --------------------------Write the complete combined flag.This is intentionally the FINAL database write for the thickness row.
        
        self.db_manager.update_a_row(self.db_name, table_name,"Date",
            database_date,
            {
                "Flag": self.combined_flag
            },
        )
        logger.warning("[FLAG] FINAL combined flag successfully written to thickness table | Table=%s | Date=%s", table_name, database_date)

    # =====================BLEND PROPERTIES ======================
    def _load_blend_properties(self):
        """Load the blend properties required by the contributor."""
        properties = (self.contributor_database.load_blend_properties(is_valid_number=self._is_valid_number))
        self.value_blend_properties.clear()
        self.data_to_be_updated.clear()
        self.value_blend_properties.update(properties.source_values)
        self.data_to_be_updated.update(properties.contributor_values)

    # ==================CONTRIBUTOR TABLE ========================
    def _write_contributor_row(self):
        """Write the contributor values to the contributor table."""

        self.contributor_database.write_contributor_row(data_to_be_updated=self.data_to_be_updated, is_valid_number=self._is_valid_number)
        
    # ====================LAB REPORT RECOVERY =====================
    def _recover_lab_report(self):
        """Recover missing Lab Report data. LabReportRecoveryManager remains responsible for the
        actual 30-calendar-day averaging calculation and database persistence.   """

        logger.info("[LAB] Starting Lab Report recovery for %s.", self.yesterday_date)
        self.lab_flag = (self.lab_report_recovery.recover_for_date(self.yesterday_date))
        if self.lab_flag:
            logger.warning("[LAB] Recovery completed | Flag=%s",self.lab_flag)
        else:
            logger.info("[LAB] No Lab Report recovery required for %s.", self.yesterday_date)

    # =====================LAB REPORT SOURCE CHECK =============================
    def _verify_lab_report_update(self):
        """Verify that all required Lab Report sections are present."""

        lab_updated, lab_table = (check_daily_lab_report_data_update(self.yesterday_date))
        if not lab_updated:
            raise RuntimeError(f"Daily lab report data is not updated for {self.yesterday_date}. Table={lab_table}" )
        logger.info("[LAB] Daily Lab Report update verified.")
    ...
    def _verify_ip21_source(self):
        ...
        """
        ip21_updated, ip21_table = (check_ip21_update(self.yesterday_date))
        if not ip21_updated:
            logger.warning("[IP21] Target date %s was not found in source table %s.", self.yesterday_date, ip21_table)
            logger.warning("[IP21] Continuing with historical recovery.")
        else:
            logger.info("[IP21] Source-date check passed.")

    # ========================FINAL IP21 VERIFICATION========================
    def _verify_final_ip21_data(self,target_datetime):
        """ Verify that exactly 24 target-date IP21 rows exist in the recovered DataFrame. """

        # ---------------------------Safety: make sure Time is datetime. ----------------------------
        self.ip21_data[
            self.TIME_COLUMN
        ] = pd.to_datetime(self.ip21_data[self.TIME_COLUMN],
            dayfirst=True,
            errors="coerce",
        )
        ...
        logger.info("[IP21] FINAL VERIFICATION | Date=%s | Target rows=%d | Recovered=%s | Flag=%s",
            self.yesterday_date,
            len(target_rows),
            self.data_was_filled_using_30_day_average,
        ...
        if len(target_rows) != 24:
            raise RuntimeError(f"Final IP21 verification failed. Expected 24 rows for {self.yesterday_date}, found {len(target_rows)}.")
        return target_rows
    ...
    def set_up(self) -> pd.DataFrame:
        ...
        self._reset_state()
        logger.info("====================================================")
        logger.info("Starting UTThicknessContributor setup | Probe ID: %s | Date: %s",self.probe_id, self.yesterday_date)
        logger.info("====================================================")
        ...
        # ======================== 4. TARGET DATETIME =========================
        target_datetime = self._get_target_datetime()
        logger.info("[IP21] Target datetime: %s", target_datetime)

        # ======================== 5. READ HISTORICAL IP21 =========================
        self.ip21_data = (self._get_ip21_historical_data(target_datetime))

        # ========================== 6. ORIGINAL IP21 STATE ========================
        self._determine_original_state(self.ip21_data,  target_datetime)

        # ======================== 7. RECOVER MISSING IP21 =========================
        self.ip21_data = (self._prepare_target_ip21_data(self.ip21_data, target_datetime))

        # ======================= 8. SAVE RECOVERED IP21 TO DATABASE =========================
        self._save_recovered_ip21_data(target_datetime)
        ...
        # 13. WRITE FINAL FLAG
        # IMPORTANT:
        # This is deliberately AFTER _write_contributor_row(). Therefore the combined Flag cannot be overwritten by the contributor database operation.
        self._write_flag_to_thickness_table()

        # ======================== 14. FINAL IP21 VERIFICATION =========================
        target_rows = (self._verify_final_ip21_data(target_datetime))

        # ======================== 15. SUCCESS =========================
        logger.info("====================================================")
        logger.info("UTThicknessContributor completed successfully | " "Probe ID: %s | Date: %s",
            self.probe_id,self.yesterday_date,
        )
        logger.info("[IP21] Recovered DataFrame is available in ""self.ip21_data.")
        logger.info("[IP21] Target-date rows available: %d",len(target_rows))
        logger.info("[Contributor] Density=%s | API=%s | Sulphur=%s",
