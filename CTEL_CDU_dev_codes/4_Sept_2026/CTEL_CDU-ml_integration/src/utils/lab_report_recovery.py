...
class LabReportRecoveryManager:
    ...
    def _recover_section(
        ...
        for column in feature_columns:
            ...
            logger.info(
                ...
                "Section=%s | Date=%s | Column=%s | Average=%s",
                display_name,
                self._format_date(
                    target_date
                ),
        ...
        #     25/11/2025
        #
        # Therefore dates are compared after _parse_date() normalization.
        # This prevents a duplicate row from being inserted.
        # ============================================================

        for (
            recovered_date,
            recovered_columns
        ) in recovered_history.items():
            ...
            try:
                ...
                # ====================================================
                #
                # Do NOT check the database using the raw string
                # "25/11/2025".
                #
                ...
                #     25/11/25 06:00
                #
                # Therefore read the Date column and compare the
                # normalized dates.
                # ====================================================
                ...
                for existing_row in existing_rows:
                    ...
                    if (
                        database_date is not None
                        and database_date == recovered_date
                    ):

                        # Keep the ACTUAL database Date value.
                        #
                        # For example:
                        #
                        #     25/11/25 06:00
                ...
                if existing_database_date is not None:
                    ...
                    #
                    # Existing valid values remain untouched.
                    # ------------------------------------------------
                    ...
                    if not result:
                        ...
                        raise RuntimeError(
                            "[LAB] Failed updating recursively "
                            "recovered historical row | "
                    ...
                    logger.info(
                        "[LAB] RECURSIVELY RECOVERED DATA UPDATED | "
                ...
                # ====================================================
                    ...
                    # ------------------------------------------------
                    # The date genuinely does not exist.
                    #
                    # Create a new Lab Report row.
                    #
                    # Only recovered columns receive values.
                    # Other feature columns remain NULL.
                    # ------------------------------------------------
                    ...
                    logger.info(
                        "[LAB] RECURSIVELY RECOVERED DATA INSERTED | "
                        "Section=%s | Table=%s | Date=%s | Columns=%s",
                        display_name,
                        recovered_table_name,
                        recovered_date_text,
    ...
    def recover_for_date(
        ...
        message = (
            ...
            f"Lab Report data was not available in "
            f"{section_text}, so Sentinal has averaged "
            f"the data of the last 30 days to predict "
            f"the Cr/Thickness"
        )
