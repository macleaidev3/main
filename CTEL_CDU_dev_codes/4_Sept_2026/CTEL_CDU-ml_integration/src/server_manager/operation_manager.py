...
class DatabaseManager:
    ...
    def _validate_primary_keys(self, primary_key_name: str, data: List[Tuple]) -> bool:
        ...
        msg.setText(f"Duplicate {primary_key_name} value(s) found:{', '.join(sorted(duplicates))}.\n\nEach {primary_key_name} must be unique before saving.")
        msg.exec()
        return False

    def _update_rows(self, rows: List[Tuple], columns: List[str], pk_column: str, pk_index: int) -> List[Tuple]:
    ...
    def create_table(self, db_name: str, table_name: str, columns: List[Tuple[str, str]]):
        ...
        try:
            ...
            for index, (name, data_type) in enumerate(columns):
                ...
                definitions.append(f"[{safe_name}] {data_type}{suffix}")
            sql = f"""
    ...
    def read_current_page_order_by(self, db_name: str, table_name: str, limit: int, offset: int, order_by_column: str = None, ascending: bool = True) -> List[Tuple]:
        ...
        try:
            ...
            if order_by_column not in columns:
                raise ValueError("Column '{order_by_column}' does not exist in '{table_name}'.")
    ...
    def save(self, db_name: str, primary_key_name: str, table_name: str, data: List[Tuple],  is_sorting_required: bool = True) -> bool:
        ...
        try:
            ...
            if inserts:
                ...
                cur.executemany(f""" INSERT INTO [{table_name}]  ({column_sql})  VALUES ({placeholders}) """, inserts)
            if updates:
                setters = ",".join(
                    f"[{c}]=?"
                    for c in columns
                ...
                cur.executemany(
                    f"""
                    UPDATE [{table_name}]   SET {setters}  WHERE [{pk_column}]=?  """,
                    self._update_rows(
                        updates,
                        columns,
                        pk_column,
    ...
    def read_columns(self, db_name: str, table_name: str, column_names: List[str]) -> List[Tuple]:
        ...
        columns = [f"[{self._sanitize_col_name(name)}]"
    ...
    def insert_columns(self, db_name: str, table_name: str, column_data: Dict[str, List[Any]]):
        ...
        try:
            cur = conn.cursor()
            table_columns = self._table_columns(cur, table_name)
            invalid = [
            ...
            cur.executemany(f"""INSERT INTO [{table_name}] ({column_sql})  VALUES ({placeholders})""", rows)
            conn.commit()
        except pyodbc.Error as exc:
            conn.rollback()
            raise RuntimeError(f"Error inserting rows into '{table_name}': {exc}") from exc
        finally:
            self._close(conn)
    ...
    def get_cell_value(self, db_name: str,table_name: str,target_column: str, pk_column: str, pk_value: Any) -> Any:
        ...
        try:
            cur = conn.cursor()
            cur.execute(f"""SELECT TOP 1 [{target}] FROM [{table_name}] WHERE [{pk}] = ? """, (pk_value,))
            row = cur.fetchone()
    ...
    def update_a_row(self, db_name: str, table: str, pk_column: str, pk_value: Any, data: Dict[str, Any]):
        """Update one row and create missing logical columns."""
        conn = self._get_connection(db_name)
        try:
            cur = conn.cursor()
            existing = set(self._table_columns(cur, table))
            for logical_column in data:
                safe_column = self._sanitize_col_name(logical_column)
                if safe_column not in existing:
                    cur.execute(f"""ALTER TABLE [{table}]  ADD [{safe_column}] NVARCHAR(MAX) """)
            setters = ",".join(f"[{self._sanitize_col_name(column)}]=?"
                for column in data
            )
            values = list(data.values()) + [pk_value]
            safe_pk = self._sanitize_col_name(pk_column)
            cur.execute(f"""UPDATE [{table}]  SET {setters} WHERE [{safe_pk}] = ? """, values)
            conn.commit()
            return True
    ...
    def read_rows_by_column_value(self, db_name: str,table_name: str, column_name: str,column_value: Any) -> List[Tuple]:
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                f"""  SELECT *  FROM [{table_name}]   WHERE [{column}] = ? """, (column_value,))
            return cur.fetchall()
        except pyodbc.Error as exc:
    ...
    def _synchronize_rows(self, db_name: str, table_name: str, data: List[Tuple],label: str) -> bool:
        ...
        try:
            ...
            update_set = ",".join(f"[{column}]=?"
                for column in update_columns
            )
            update_sql = f""" UPDATE [{table_name}]  SET {update_set} WHERE [{pk_column}]=?  """
            inserts = []
            updates = []
            for row in data:
                key = row[pk_index]
            ...
            conn.rollback()
            logger.exception("%s synchronization failed.",  label)
            return False
        finally:
            self._close(conn)

    def synchronize_save_ip21(self, db_name: str, primary_key_name: str, table_name: str,  data: List[Tuple]) -> bool:
        """Synchronize IP21 without deleting historical rows."""
        if not self._validate_primary_keys(primary_key_name, data):
            logger.error("IP21 synchronization aborted due to duplicate primary keys.")
            return False
        return self._synchronize_rows(db_name, table_name, data,  "IP21")

    def synchronize_save_lab(self, db_name: str, primary_key_name: str, table_name: str, data: List[Tuple]) -> bool:
        """Synchronize laboratory data without deleting rows."""
        if not self._validate_primary_keys(primary_key_name, data):
            logger.error("Laboratory synchronization aborted due to duplicate primary keys.")
            return False
        return self._synchronize_rows(db_name, table_name, data, "Laboratory")
    ...
    def special_method_ip_21_get_rows_for_day(self, date_str: str) -> Dict[str, List[Any]]:
        ...
        table_name = (f"ip21_{target_date.year}_{month}")
        ...
        try:
            cur = conn.cursor()
            cur.execute(f""" SELECT *  FROM [{table_name}] WHERE col_Time LIKE ?  ORDER BY col_Time ASC""",
                (f"{date_prefix}%",),
            )
            rows = cur.fetchall()
            result = {
