    ...
    while True:
        ...
        try:
            ...
            # ---------------------- Manual job---------------------------------------
                ...
                flagged_dates = _run_manual_job(instruments, dates)
            else:
            ...
            logger.info("[Worker] Job %s completed successfully. Flagged dates: %s", job_id, flagged_dates)
...
def _run_automatic_job(dates):
    ...
    contributor_instance = UTThicknessContributor00001(month=month, year=year, yesterday_date=yesterday_date)
    _collect_combined_flag(flagged_dates, "00001", yesterday_date, contributor_instance)
    ...
    contributor_instance = UTThicknessContributor00003(month=month, year=year, yesterday_date=yesterday_date)
    _collect_combined_flag(flagged_dates, "00003", yesterday_date, contributor_instance)
    ...
    contributor_instance = UTThicknessContributor00004(month=month, year=year, yesterday_date=yesterday_date)
    _collect_combined_flag(flagged_dates, "00004", yesterday_date, contributor_instance)
    ...
    contributor_instance = UTThicknessContributor00005(month=month, year=year, yesterday_date=yesterday_date)
    _collect_combined_flag(flagged_dates, "00005", yesterday_date, contributor_instance)
    ...
    contributor_instance = UTThicknessContributor00006(month=month, year=year, yesterday_date=yesterday_date)
    _collect_combined_flag(flagged_dates, "00006", yesterday_date, contributor_instance)
    ...
    contributor_instance = UTThicknessContributor00029(month=month, year=year, yesterday_date=yesterday_date)
    _collect_combined_flag(flagged_dates, "00029", yesterday_date, contributor_instance)
    ...
    contributor_instance = UTThicknessContributor00030(month=month, year=year, yesterday_date=yesterday_date)
    _collect_combined_flag(flagged_dates, "00030", yesterday_date, contributor_instance)
    UTThicknessPrediction00030(month=month, year=year, yesterday_date=yesterday_date)

    # ----------------------------- Return all collected flags--------------------------------------------------------
    logger.info("[Worker] AUTOMATIC FINAL flagged_dates = %s", flagged_dates)
    return flagged_dates

# ======================= Flag collection helper==============================================
def _collect_combined_flag(flagged_dates, instrument, date, contributor_instance):
    """
    Collect the combined flag generated by the contributor.
    The contributor is responsible for creating the actual IP21 + Lab Report flag.
    Example returned flag:
    15/08/2025 IP21 data was not available, so Sentinal has averaged the data of the last 30 days to predict the Cr/Thickness
    OR:
    15/08/2025 Lab Report data was not available in section "AD Stage-1", so Sentinal has averaged the data of the last 30 days to predict the Cr/Thickness
    OR both messages together.
    """

    combined_flag = getattr(contributor_instance, "combined_flag", None)
    if combined_flag:
        instrument = str(instrument)
        date = str(date).strip()
        flagged_dates.setdefault(instrument, {})[date] = combined_flag
        logger.warning("[Worker] FLAGGED | Instrument=%s | Date=%s | %s", instrument, date, combined_flag)
    else:
        logger.info("[Worker] No combined flag | Instrument=%s | Date=%s", instrument, date)

# ====================== Manual ML job======================================================
def _run_manual_job(instruments, dates):
    """
    Manual job. Runs ML only for selected instruments and dates.
    Returns
    -------
    dict
    ...
    # ---------------------------- Existing database manager used by the existing prediction-input checking logic.--------------------------------------------
    ...
    # This is retained. It is NOT the Lab Report recovery logic.
    # ========================================================
    def check_missing_prediction_data(instrument, date):
        """
        Check whether the existing UT prediction inputs are missing for the selected prediction date.
        This existing check is retained and does not modify the Lab Report recovery logic.
        """
        required_properties = ["Density(g/ml)", "API", "Sulphur%" ]
        day, month_i, year = map( int, date.split("/"))
        month = month_short_name()[month_i - 1]
        contributor_table = (f"ut_{instrument}_{year}_{month}_contributor")
        missing_properties = []
        for prop in required_properties:
            try:
                value = db_manager.get_cell_value(db_name,contributor_table, prop, "Date", date)
                if (
                    value is None
                    or (isinstance(value, str) and value.strip().lower()
                        in {
                            "",
                            "nan",
                            "none",
                    ...
                    or (not isinstance(value, str) and value != value )
                ):
                    missing_properties.append(prop)

            except Exception:
                logger.exception("[Worker] Failed to check prediction input '%s' for instrument %s on %s.", prop, instrument, date)
                missing_properties.append(prop)

        if missing_properties:
            logger.warning("[Worker] Missing prediction data detected.Instrument: %s | Date: %s | Missing: %s", instrument, date,
                missing_properties
            )
            return True
        return False
    ...
    for instrument in instruments:
        ...
        # ======================== Pipeline: IC-E-102 -> IC-E-161 A~H===================================================
        ...
        # ============================= Pipeline: IC-V-112 -> IC-E-162 A~P==================================================== 
        ...
        # =========================== Pipeline: IC-E-126 A~D -> IC-V-113 ====================================================
        ...
        # ========================= Pipeline: IC-E-161 A~H -> IC-V-112 ====================================================
        ...
        # ========================== Pipeline: IC-E-162 A~P -> IC-E-126 A~D=============================================
            ...
            logger.warning("[Worker] No contributor configured for instrument: %s",instrument)
            continue
        ...
        for date in dates:
            ...
            # ------------------- Existing prediction-input check. Keep this because it was already part of your application. ------------------------------------------------
            if instrument in {
                "00001", "00003", "00004",
                "00005", "00006", "00029",
                "00030"
            }:
                was_missing_before_contributor = (check_missing_prediction_data(instrument, date))
                logger.info("[Worker] Missing-data check BEFORE contributor | Instrument=%s | Date=%s | was_missing=%s",
                    instrument, date, was_missing_before_contributor
                )
            ...
            # ------------------------------------------------
            contributor_instance = _contributor(month=month, year=year, yesterday_date=date)

            # ------------------------------------------------
            # Get combined flag generated by contributor.
            # This is where the Lab Report flag and existing IP21 flag reach ml_job.py.
            # ------------------------------------------------
            _collect_combined_flag(flagged_dates, instrument, date,  contributor_instance )

            # --------------------- Existing prediction-input check AFTER contributor ------------------------------------------------
            if instrument in {
                "00001",  "00003", "00004",  "00005",
                "00006",  "00029", "00030",
            }:
                was_missing_after_contributor = (check_missing_prediction_data(instrument, date))
                logger.info("[Worker] Missing-data check AFTER contributor | Instrument=%s | Date=%s | was_missing=%s",
                    instrument,  date,  was_missing_after_contributor
                )

            # ----------------------- Run prediction This is unchanged. ------------------------------------------------
            if _calculator is not None:
                _calculator(month=month, year=year, yesterday_date=date)
            else:
                logger.warning("[Worker] No prediction calculator configured for Instrument %s | Date %s",
                    instrument, date
                )
    # =========================== Return flags to worker_loop ========================================================
    logger.info("[Worker] MANUAL FINAL flagged_dates = %s", flagged_dates)
    return flagged_dates
...
class MidnightFetcherMultiprocess(QtCore.QObject):
    ...
    def __init__(self, interval_minutes: float, parent=None):
        ...
        self.task_q = Queue()
        self.result_q = Queue()
        self.worker = Process(target=worker_loop, args=(self.task_q,self.result_q), daemon=True)
        self.worker.start()
    ...
    def _enqueue_automatic_job(self):
        yesterday = (QtCore.QDate.currentDate().addDays(-1).toString("dd/MM/yyyy"))
        job = {
    ...
    def _poll_results(self):
        ...
        while not self.result_q.empty():
            ...
            # ------------------------------------------------
            # Manual result
            # Send complete result, including flagged_dates, to the UI.
            # ------------------------------------------------
            if result["type"] == "manual":
                logger.info("[Fetcher] Sending manual job result to UI. Flagged dates: %s", result.get("flagged_dates", {}))
