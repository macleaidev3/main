...
class ID00030Model:
    def __init__(
    self,
    artifact_dir=resource_path("ml_module/cache_mlmodule_1232232/ID_00030"),
    historical_path=resource_path("ml_module/cache_mlmodule_1232232/ID_00030/test_original.csv")
):
        """
        Initializes the model, loads scalers, and pre-loads the historical data into memory.
        """

        logger.info("Initializing ID00030 corrosion probe prediction model.")
        ...
        try:
            ...
            logger.info(
                "ID00030 corrosion probe prediction model initialized successfully."
            )

        except Exception:
            logger.exception(
                "Failed to initialize ID00030 corrosion probe prediction model."
            )
            raise
