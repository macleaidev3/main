...
class ID00003Model:
    def __init__(
        self,
        artifact_dir=resource_path("ml_module/cache_mlmodule_1232232/ID_00003"),
        historical_path=resource_path(
            "ml_module/cache_mlmodule_1232232/ID_00003/test_original.csv"
        ),
    ):
        """
        ...
        logger.info("Initializing ID00003 corrosion probe prediction model.")
        ...
        try:
            ...
            self.feature_scaler = joblib.load(
                f"{self.artifact_dir}/feature_scaler.pkl"
            )
            self.target_scaler = joblib.load(
                f"{self.artifact_dir}/target_scaler.pkl"
            )
            ...
            logger.info(
                "ID00003 corrosion probe prediction model initialized successfully."
            )

        except Exception:
            logger.exception(
                "Failed to initialize ID00003 corrosion probe prediction model."
            )
            raise
    ...
    def _initialize_historical_data(self):
        ...
        hist["time_diff_hours"] = (
            hist["Sent Time"].diff().dt.total_seconds() / 3600
        )
        ...
        hist["corrosion_rate"] = (
            hist["thickness_diff"] / hist["time_diff_hours"]
        )
        ...
        # Save as the active state dataframe
    ...
    def predict_single_instance(self, input_tuple):
        ...
        """
        logger.debug(f"Received prediction request: {input_tuple}")
        ...
        try:
            ...
            new_row = pd.DataFrame([{
                ...
                "API": api,
                "Sulphur": sulphur,
                "thickness": np.nan  # Unknown, to be predicted
            }])

            # ---------------------------------------------------------
            # 2. Append to a TEMPORARY dataframe to protect class state
            # ---------------------------------------------------------
            temp_data = pd.concat(
            ...
            # Forward fill thickness (uses last known/predicted thickness)
            ...
            # ---------------------------------------------------------
            # 9. Calculate predicted thickness
            # ---------------------------------------------------------
            ...
            # ---------------------------------------------------------
            # 10. Commit prediction to temporary state
            # ---------------------------------------------------------
            temp_data.loc[
                len(temp_data) - 1,
                "thickness"
            ...
            logger.exception(
                f"Prediction failed for input: {input_tuple}"
            )
            return None