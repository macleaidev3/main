...
class CrCalculationInterface(QtWidgets.QWidget):
    ...
    def on_calculation_completed(self, job):
        ...
        for probe_id, dates in (flagged_dates or {}).items():

            probe_id = str(probe_id).strip()
            # ---------------------------------------------------------
            # The worker's flagged_dates represents the CURRENT
            # prediction result for this instrument.
            #
            # Therefore, replace the previous list instead of
            # continuously accumulating old flags.
            # --------------------------------------------------------
            ...
            for date in (dates or []):
                ...
                if (
                    date
                    and date not in current_flagged_dates
                ):
                    current_flagged_dates.append(date)

            self.flagged_dates[probe_id] = current_flagged_dates

            logger.info(
                "[FLAG DEBUG] Current flags for instrument %s: %s",
                probe_id,
                current_flagged_dates
            )            
        ...
        if application is not None:
            ...
            for table in application.allWidgets():
                ...
                # ---------------------------------------------------------
                # Always call set_flag_messages(), even when the list is
                # empty.
                #
                # An empty list means:
                #
                #     "This instrument has NO flags in the latest prediction."
                #
                # UTThicknessTable will therefore clear any previous flags.
                # ---------------------------------------------------------
