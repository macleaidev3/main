...
class AutoFeatureBuilder(BaseEstimator, TransformerMixin):
    ...
    def fit(
        ...
        self.output_features_ = list(
            self._build(X).columns
        )
    ...
    def _build(
        ...
        self._add_skew_transforms(
            df,
            out,
        )

        self._add_physics_features(
            df,
            out,
        )

        self._add_simulation_context(
            source,
            df,
            out,
        )
    ...
    def _add_geometry(
        ...
        for col in self.coord_columns_:
            ...
            centered[col] = (
                df[col] - self.centers_[col]
            ) / self.scales_[col]

            out[f"{name}_centered"] = centered[col]

            out[f"{name}_centered_sq"] = (
                centered[col] ** 2
            )
        ...
        for left, right in (
            ...
            if left in centered and right in centered:
                ...
                out[
                    f"{self._short_name(left)}_{self._short_name(right)}"
                ] = (
                    centered[left]
                    * centered[right]
                )
        ...
        if set(COORDS).issubset(centered):
            ...
            stacked = np.vstack(
                [
                    centered[col].to_numpy()
                    for col in COORDS
                ]
            )

            out["radius_xyz"] = np.sqrt(
                np.sum(
                    stacked ** 2,
                    axis=0,
                )
    ...
    def _ratio(
        ...
        if numerator in df and denominator in df:
            ...
            den = df[denominator].where(
                df[denominator].abs() > 1e-12
            )
...
class TotalPressurePredictor:
    ...
    MODEL_PATH = resource_path(
        "ml_module/cache_mlmodule_1232232/ICE161/total_pressure/final_model.joblib"
    )
    ...
    def __init__(self):
        ...
        self.features = artifact["predictors"]

        self.builder_state = artifact[
            "feature_builder_state"
        ]

        self.imputer = artifact["imputer"]

        self.poly = artifact["polynomial"]

        self.variance = artifact["variance"]

        self.scaler = artifact["scaler"]

        self.corr_state = artifact[
            "correlation_state"
        ]

        self.selector_state = artifact[
            "selector_state"
        ]
        ...
        self.target_column = artifact.get(
            "target",
            "Predicted_TotalPressure_Pa",
        )

        #
        # Restore AutoFeatureBuilder
        #

        self.builder = AutoFeatureBuilder()

        self.builder.numeric_columns_ = (
            self.builder_state[
                "numeric_columns"
            ]
        )

        self.builder.coord_columns_ = (
            self.builder_state[
                "coord_columns"
            ]
        )

        self.builder.centers_ = (
            self.builder_state[
                "centers"
            ]
        )

        self.builder.scales_ = (
            self.builder_state[
                "scales"
            ]
        )

        self.builder.skewed_columns_ = (
            self.builder_state[
                "skewed_columns"
            ]
        )

        self.builder.context_columns_ = (
            self.builder_state[
                "context_columns"
            ]
        )

        self.builder.output_features_ = (
            self.builder_state[
                "output_features"
            ]
        )
        ...
        #
        # Restore CorrelationFilter
        #

        self.corr = CorrelationFilter()

        self.corr.support_ = (
            self.corr_state["support"]
        )

        #
        # Restore ImportanceSelector
        #

        self.selector = ImportanceSelector()

        self.selector.support_ = (
            self.selector_state["support"]
        )
    ...
    def predict(
        ...
        #
        # Training used a simulation column
        #
        ...
        X_processed = self.builder.transform(
            X
        )
        ...
        X_processed = self.imputer.transform(
            X_processed
        )
        ...
        X_processed = self.poly.transform(
            X_processed
        )
        ...
        X_processed = self.variance.transform(
            X_processed
        )
        ...
        X_processed = self.corr.transform(
            X_processed
        )
        ...
        X_processed = self.selector.transform(
            X_processed
        )
