"""
Total Temperature prediction module.
...
training, predicts Total Temperature, and returns the prediction without
...
MODEL_PATH = resource_path(
    "ml_module/cache_mlmodule_1232232/ICV113/total_temperature.joblib"
)


class TotalTemperaturePredictor:
    """
    Predictor for Total Temperature.
    """
    ...
    def _validate_features(self, df: pd.DataFrame) -> None:
        ...
        if missing:
            raise ValueError(
                "Missing input features for Total Temperature prediction: "
                f"{missing}"
            )